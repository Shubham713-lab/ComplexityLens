...
export default function ThemeToggle({ theme, setTheme, className = '' }) {
  ...
  return (
    <button
      ...
      className={`relative inline-flex items-center h-7 w-14 rounded-md p-1 bg-stone-200 dark:bg-zinc-900 border border-stone-300 dark:border-zinc-700/80 hover:border-orange-600/50 dark:hover:border-orange-500/50 transition-colors duration-300 cursor-pointer focus:outline-none shrink-0 ${className}`}
    ...
    >
      ...
      {/* Sliding Rounded Square Knob */}
      <span
        className={`absolute top-1 left-1 w-5 h-5 rounded transform transition-transform duration-300 ease-in-out flex items-center justify-center border shadow-xs ${
          isDark
