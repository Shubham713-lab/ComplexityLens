...
export default function LineCostTable({ lineCosts }) {
  ...
  return (
    <div className="glass-panel h-full flex-1 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl flex flex-col min-h-0 bg-white/80 dark:bg-slate-950/60">
      <div className="px-5 py-2.5 bg-slate-100/90 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <ListTree className="w-4 h-4 text-cyan-600 dark:text-cyan-400" />
          <h3 className="text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">
            Line Cost Breakdown
          </h3>
        </div>
        <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{costList.length} lines</span>
      </div>

      <div className="flex-1 overflow-y-auto divide-y divide-slate-200 dark:divide-slate-800/60 font-mono text-xs sm:text-sm min-h-0">
        {costList.map((item, idx) => (
          <div
            key={idx}
            className="px-5 py-2 flex items-center justify-between gap-4 hover:bg-slate-100/70 dark:hover:bg-slate-800/40 transition-colors"
          >
            {/* Line Number & Text */}
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <span className="w-6 text-right text-slate-400 dark:text-slate-500 select-none font-bold text-xs">{item.line}</span>
              <code className="text-slate-800 dark:text-slate-200 truncate font-mono text-xs sm:text-sm" title={item.text}>
                {item.text}
              </code>
            </div>
